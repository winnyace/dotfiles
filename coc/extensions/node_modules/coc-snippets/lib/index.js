...
var LanguageProvider = class {
  ...
  async resolveCompletionItem(item) {
    ...
    let snip = await Promise.resolve(import_coc.snippetManager.resolveSnippet(text, true));
    item.documentation = {
      kind: "markdown",
...
var SnipmateProvider = class extends BaseProvider {
  ...
  async init() {
    ...
    await Promise.all(Array.from(import_coc4.workspace.filetypes).map((filetype) => {
...
var TextmateProvider = class extends BaseProvider {
  ...
  async getSnippetFiles(filetype) {
    ...
    for (let filetype2 of filetypes) {
      let snippets = this.loadedSnippets.filter((s) => s.filetype == filetype2);
      if (snippets.length) {
        for (let snip of snippets) {
          let { filepath } = snip;
  ...
  async getSnippets(filetype) {
    ...
    for (let filetype2 of filetypes) {
      let snippets = this.loadedSnippets.filter((o) => o.filetype == filetype2);
      if (snippets && snippets.length) {
        for (let snip of snippets) {
    ...
    if (packageJSON.contributes && packageJSON.contributes.snippets) {
      ...
      for (let item of snippets) {
        ...
        if (import_fs3.default.existsSync(p)) {
          let languages3 = typeof item.language == "string" ? [item.language] : item.language;
          arr.push({
            languageIds: languages3,
            filepath: p
          });
        }
      }
  ...
  async loadDefinitionFromRoot(root) {
    ...
    for (let file of files) {
      ...
      if (file.endsWith(".code-snippets")) {
        let extensionId = import_path3.default.basename(root);
        await this.loadSnippetsFromFile(filepath, void 0, extensionId);
      } else {
  ...
  async loadSnippetsFromFile(snippetFilePath, languageIds, extensionId) {
    if (this.loadedFiles.has(snippetFilePath))
      return;
  ...
  loadSnippetsFromText(filepath, extensionId, ids, contents) {
    ...
    snippets.forEach((snip) => {
      if (!snip.prefix)
        return;
      let languageIds = snip.scope ? snip.scope.split(",") : void 0;
...
var import_pify2 = __toESM(require_pify());
...
var UltiSnipsParser = class {
  ...
  async execute(code, pyMethod, indent) {
    ...
    if (code.startsWith("!")) {
      ...
      try {
        res = await (0, import_pify2.default)(import_child_process.exec)(code);
      } catch (e) {
        res = `Error: ${e.message}`;
...
var UltiSnippetsProvider = class extends BaseProvider {
  constructor(channel, trace, config, context) {
    ...
    this.snippetFiles = [];
    this.fileItems = [];
    import_coc7.workspace.onDidSaveTextDocument(async (doc) => {
      let uri = import_coc7.Uri.parse(doc.uri);
  ...
  get directories() {
    ...
    return dirs.map((dir) => import_coc7.workspace.expand(dir));
  }
  async init() {
    let { nvim, env } = import_coc7.workspace;
    let { config } = this;
    ...
    import_coc7.workspace.onDidRuntimePathChange(async (e) => {
    ...
    await import_coc7.workspace.nvim.command(`exe '${this.pyMethod}file '.fnameescape('${filepath}')`);
    const items = this.getValidItems(this.fileItems);
    if (items.length)
      await this.loadFromItems(items);
    import_coc7.workspace.onDidOpenTextDocument(async (e) => {
      let doc = import_coc7.workspace.getDocument(e.bufnr);
      if (doc)
        await this.loadByFiletype(doc.filetype);
    }, null, this.context.subscriptions);
  }
  ...
  get allFiletypes() {
    let filetypes = Array.from(import_coc7.workspace.filetypes);
    let res = [];
...
` + code + "\n";
  ...
  async checkContext(context) {
    ...
    await this.executePyCodes(pyCodes);
    return await import_coc7.workspace.nvim.call(`${this.pyMethod}eval`, "True if context else False");
  }
  async executePyCodes(lines) {
    await import_coc7.workspace.nvim.command(`${this.pyMethod} ${addPythonTryCatch(lines.join("\n"))}`);
  }
  async getTriggerSnippets(document, position, autoTrigger) {
    let snippets = await this.getSnippets(document.filetype);
    let line = document.getline(position.line);
    ...
    for (let s of snippets) {
      ...
      let range = import_coc7.Range.create(position.line, character, position.line, position.character);
      let newText = await this.resolveSnippetBody(s, range, line);
      edits.push({
        prefix: s.prefix,
        description: s.description,
  ...
  async getSubFolders() {
    ...
    let dirs = await import_coc7.workspace.nvim.eval('get(g:, "UltiSnipsSnippetDirectories", [])');
    for (let dir of dirs) {
  ...
  async executePythonCode(pythonCode) {
    try {
      ...
      await import_coc7.workspace.nvim.command(`exe '${this.pyMethod}file '.fnameescape('${tmpfile}')`);
    } catch (e) {
      this.channel.appendLine(`Error on execute python script:`);
      this.channel.append(e.message);
      import_coc7.window.showMessage(`Error on execute python script: ${e.message}`, "error");
    }
  }
};
...
function addPythonTryCatch(code) {
  if (!import_coc7.workspace.isVim)
    return code;
  let lines = [
...
async function insertSnippetEdit(edit) {
  await import_coc8.commands.executeCommand("editor.action.insertSnippet", import_coc8.TextEdit.replace(edit.range, edit.newText), edit.source === "ultisnips");
}
function enableSnippetsFiletype(subscriptions) {
  let { nvim } = import_coc8.workspace;
  import_coc8.workspace.documents.forEach((doc) => {
    if (doc.uri.endsWith(".snippets")) {
      doc.buffer.setOption("filetype", "snippets", true);
    }
  });
  import_coc8.workspace.onDidOpenTextDocument(async (document) => {
    if (document.uri.endsWith(".snippets")) {
      let doc = import_coc8.workspace.getDocument(document.uri);
      if (!doc)
        return;
      let { buffer } = doc;
  ...
  const rtp = import_coc8.workspace.env.runtimepath;
  let paths = rtp.split(",");
...
async function snippetSelect() {
  let doc = await import_coc8.workspace.document;
  if (!doc)
    return;
  let { nvim } = import_coc8.workspace;
  let mode = await nvim.call("visualmode");
  if (["v", "V"].indexOf(mode) == -1) {
    import_coc8.window.showMessage(`visual mode ${mode} not supported`, "warning");
    return;
  }
  await nvim.command("normal! `<");
  let start = await import_coc8.window.getCursorPosition();
  await nvim.command("normal! `>");
  let end = await import_coc8.window.getCursorPosition();
  end = import_coc8.Position.create(end.line, end.character + 1);
  let range = import_coc8.Range.create(start, end);
  ...
  if (mode == "v") {
    ...
    text = lines.join("\n");
    range = import_coc8.Range.create(import_coc8.Position.create(start.line, indent.length), end);
    await doc.applyEdits([{ range, newText: "" }]);
  }
  await nvim.setVar("coc_selected_text", text);
  await import_coc8.window.moveTo(range.start);
}
async function activate(context) {
  var _a;
  let { subscriptions } = context;
  const { nvim } = import_coc8.workspace;
  ...
  const channel = import_coc8.window.createOutputChannel("snippets");
  ...
  if (!Array.isArray(excludes))
    excludes = [];
  excludes = excludes.map((p) => import_coc8.workspace.expand(p));
  ...
  if (configuration.get("autoTrigger", true)) {
    import_coc8.events.on("TextInsert", async (bufnr, info) => {
      ...
      if (edits.length > 1) {
        ...
        import_coc8.window.showMessage("Multiple snippets found on auto trigger", "warning");
        await import_coc8.commands.executeCommand("workspace.showOutput", "snippets");
      }
  ...
  if (manager.hasProvider) {
    let disposable = import_coc8.languages.registerCompletionItemProvider("snippets", configuration.get("shortcut", "S"), null, manager, configuration.get("triggerCharacters", []), configuration.get("priority", 90));
  ...
  async function doExpand(bufnr) {
    ...
    if (edits.length == 1) {
      ...
      let idx = await import_coc8.window.showQuickpick(edits.map((e) => e.description || e.prefix), "choose snippet:");
      if (idx == -1)
        return;
  ...
  subscriptions.push(import_coc8.workspace.registerKeymap(["x"], "convert-snippet", async () => {
    let mode = await import_coc8.workspace.nvim.call("visualmode");
    if (!mode)
      return;
    let doc = await import_coc8.workspace.document;
    if (!doc)
      return;
    let range = await import_coc8.window.getSelectedRange(mode);
    let text = doc.textDocument.getText(range);
    if (text)
      await import_coc8.commands.executeCommand("snippets.editSnippets", text);
  }, { sync: false }));
  subscriptions.push(import_coc8.commands.registerCommand("snippets.editSnippets", async (text) => {
  ...
  subscriptions.push(import_coc8.commands.registerCommand("snippets.openSnippetFiles", async () => {
    let buf = await nvim.buffer;
    let doc = import_coc8.workspace.getDocument(buf.id);
    if (!doc) {
      import_coc8.window.showMessage("Document not found", "error");
    ...
    if (!files.length) {
      import_coc8.window.showMessage("No related snippet file found", "warning");
      return;
    }
    let idx = await import_coc8.window.showQuickpick(files, "choose snippet file:");
    if (idx == -1)
      return;
    let uri = import_coc8.Uri.file(files[idx]).toString();
    await import_coc8.workspace.jumpTo(uri, null, configuration.get("editSnippetsCommand"));
  }));
  subscriptions.push(import_coc8.workspace.registerKeymap(["i"], "snippets-expand", async () => {
    let bufnr = await nvim.eval('bufnr("%")');
    let expanded = await doExpand(bufnr);
    if (!expanded)
      await fallback();
  }, { silent: true, sync: true, cancel: true }));
  subscriptions.push(import_coc8.workspace.registerKeymap(["i"], "snippets-expand-jump", async () => {
    let bufnr = await nvim.eval('bufnr("%")');
    let expanded = await doExpand(bufnr);
    if (!expanded) {
      let session = import_coc8.snippetManager.getSession(bufnr);
      if (session && session.isActive) {
        await nvim.call("coc#_cancel", []);
        await import_coc8.snippetManager.nextPlaceholder();
        return;
      }
      await fallback();
    }
  }, { silent: true, sync: true, cancel: true }));
  subscriptions.push(import_coc8.workspace.registerKeymap(["v"], "snippets-select", snippetSelect, { silent: true, sync: false, cancel: true }));
  let languageProvider = new LanguageProvider(channel, trace);
  subscriptions.push(import_coc8.languages.registerCompletionItemProvider("snippets-source", configuration.get("shortcut", "S"), ["snippets"], languageProvider, ["$"], configuration.get("priority", 90)));
  subscriptions.push(channel);
  subscriptions.push(import_coc8.listManager.registerList(new SnippetsList(import_coc8.workspace.nvim, manager)));
  return {
    expandable: async () => {
      let bufnr = await nvim.eval('bufnr("%")');
      let edits = await manager.getTriggerSnippets(bufnr);
